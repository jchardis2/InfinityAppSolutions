...
import com.infinityappsolutions.server.webvideo.loaders.VideoLoader;
...
public class VideoDAO {
	...
	public void editVideo(VideoBean video) throws DBException {
		...
		try {
			conn = factory.getConnection();
			ps = conn
					.prepareStatement("UPDATE  `video` SET  `id` =?,`name` =?,`type`=?,`url`=? WHERE  `video`.`id` =?;");
	...
	public void insertVideo(VideoBean video) throws DBException {
		...
		try {
			conn = factory.getConnection();
			ps = conn
					.prepareStatement("INSERT INTO  `video` (`id` ,`name` ,`type` ,`url`)VALUES (? ,  ?,  ?,  ?);");
			videoLoader.loadParameters(ps, video);
			ps.executeUpdate();
		} catch (SQLException | NamingException e) {
