...
import com.infinityappsolutions.webdesigner.loaders.OrgUserLoader;
...
public class OrgUserDAO {
	 ...
	 * @throws DBException
	 */
	public ArrayList<OrgUserBean> getOrgUsers(long id) throws WebDesignerException, DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("SELECT `org_users` . * , `org`.`name`  FROM org_users, org WHERE  `userid` =? AND  `org_users`.`orgid` =  `org`.`id` ");
			ps.setLong(1, id);
			ResultSet rs = ps.executeQuery();
			ArrayList<OrgUserBean> orgUsersList = (ArrayList<OrgUserBean>) orgUserLoader.loadList(rs);
			rs.close();
			ps.close();
			return orgUsersList;
	...
	public int updateOrgUsers(OrgUserBean oub) throws WebDesignerException, DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
	...
	public int insertOrgUsers(OrgUserBean oub) throws WebDesignerException, DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			ps = conn
					.prepareStatement("INSERT INTO  `webdesigner`.`org_users` (`orgid` ,`userid` ,	`createProjects` , `deleteProjects` , `editProjects` , `adduser` , `addadmin` , `editAllProjects` )	VALUES ( '1',  '1',  '0',  '0',  '0',  '0',  '0',  '0');");
			orgUserLoader.loadParameters(ps, oub);
			int rs = ps.executeUpdate();
			ps.close();
			return rs;
	...
	public int insertOrgUsersList(ArrayList<OrgUserBean> oub) throws WebDesignerException, DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
